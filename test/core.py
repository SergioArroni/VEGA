...
class Problem:
    ...
    def __call__(self, solution):
        ...
        solution.variables[:] = [
            problem.types[i].decode(solution.variables[i]) for i in range(problem.nvars)
        ]

        self.evaluate(solution)

        solution.variables[:] = [
            problem.types[i].encode(solution.variables[i]) for i in range(problem.nvars)
        ]
        solution.constraint_violation = sum(
            [
                abs(f(x))
                for (f, x) in zip(solution.problem.constraints, solution.constraints)
            ]
        )
    ...
    def evaluate(self, solution):
        ...
        if len(objs) != self.nobjs:
            raise PlatypusError(
                "incorrect number of objectives: expected %d, received %d"
                % (self.nobjs, len(objs))
            )

        if len(constrs) != self.nconstrs:
            raise PlatypusError(
                "incorrect number of constraints: expected %d, received %d"
                % (self.nconstrs, len(constrs))
            )
...
class Algorithm:
    ...
    def __init__(self, problem, evaluator=None, log_frequency=None, **kwargs):
        super().__init__()
    ...
    def evaluate_all(self, solutions):
        ...
        for i, result in enumerate(results):
            if unevaluated[i] != result.solution:
                ...
                unevaluated[
                    i
                ].constraint_violation = result.solution.constraint_violation
    ...
    def run(self, condition, callback=None):
        ...
        while not condition(self):
            ...
            if (
                ...
                and self.nfe >= last_log + self.log_frequency
            ):
                LOGGER.log(
                    logging.INFO,
        ...
        if value is not None:
            ...
            if op[1] == "=":
                self.function = functools.partial(
                    Constraint.OPERATORS[op[0:2]], y=float(op[2:])
                )
            else:
                self.function = functools.partial(
                    Constraint.OPERATORS[op[0:1]], y=float(op[1:])
                )
...
class Solution:
    ...
    def __str__(self):
        return (
            "Solution["
            + ",".join(list(map(str, self.variables)))
            + "|"
            + ",".join(list(map(str, self.objectives)))
            + "|"
            + str(self.constraint_violation)
            + "]"
        )
...
class ParetoDominance(Dominance):
    ...
    def compare(self, solution1, solution2):
        ...
        if (
            problem.nconstrs > 0
            and solution1.constraint_violation != solution2.constraint_violation
        ):
...
class FitnessArchive(Archive):
    ...
    def truncate(self, size):
        ...
        self._contents = truncate_fitness(
            self._contents,
            size,
...
class EpsilonBoxArchive(Archive):
    ...
    def add(self, solution):
        ...
        not_same_box = [
            not self._dominance.same_box(solution, s) for s in self._contents
        ]
...
def unique(solutions, objectives=True):
    ...
    for solution in solutions:
        ...
        if objectives:
            ...
            id = tuple(
                [
                    problem.types[i].decode(solution.variables[i])
    ...
    if len(solutions) < 3:
        ...
        for i in range(nobjs):
            sorted_solutions = sorted(
                solutions, key=functools.partial(objective_key, index=i)
            )
...
def normalize(solutions, minimum=None, maximum=None):
    ...
    if minimum is None or maximum is None:
        if minimum is None:
            minimum = [
                min([s.objectives[i] for s in feasible]) for i in range(problem.nobjs)
            ]

        if maximum is None:
            maximum = [
                max([s.objectives[i] for s in feasible]) for i in range(problem.nobjs)
            ]
    ...
    for s in feasible:
        s.normalized_objectives = [
            (s.objectives[i] - minimum[i]) / (maximum[i] - minimum[i])
            for i in range(problem.nobjs)
        ]
    ...
    def evaluate(self, solutions):
        ...
        for i in range(len(solutions)):
            ...
            for j in range(len(solutions)):
                if i != j:
                    sum += math.exp(
                        (-self.fitcomp[j][i] / self.max_fitness) / self.kappa
                    )
    ...
    def remove(self, solutions, index):
        for i in range(len(solutions)):
            if i != index:
                ...
                fitness -= math.exp(
                    (-self.fitcomp[index][i] / self.max_fitness) / self.kappa
                )
...
class HypervolumeFitnessEvaluator(FitnessEvaluator):
    def __init__(self, kappa=0.05, rho=2.0, dominance=ParetoDominance()):
    ...
    def hypervolume(self, solution1, solution2, d):
        ...
        if d == 1:
            ...
            if a < b:
                return (
                    self.hypervolume(solution1, None, d - 1) * (b - a) / self.rho
                    + self.hypervolume(solution1, solution2, d - 1)
                    * (self.rho - b)
                    / self.rho
                )
            else:
