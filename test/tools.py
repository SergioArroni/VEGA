...
def point_line_dist(point, line):
    return magnitude(
        subtract(
            multiply(float(dot(line, point)) / float(dot(line, line)), line), point
        )
    )
...
class DistanceMatrix:
    ...
    def remove_point(self, index):
        ...
        for i in range(len(self.distances)):
            self.distances[i] = [
                (x if x < index else x - 1, y)
                for (x, y) in self.distances[i]
                if x != index
            ]
...
def check_eigensystem(n, C, diag, Q):
    ...
    for i in range(n):
        for j in range(n):
            ...
            if (
                abs(cc - C[i if i > j else j][j if i > j else i])
                / math.sqrt(C[i][i] * C[j][j])
                > 1e-10
                and abs(cc - C[i if i > j else j][j if i > j else i]) > 1e-9
            ):
                print >> sys.stderr, "imprecise result detected", i, j, cc, C[
                    i if i > j else j
                ][j if i > j else i], (cc - C[i if i > j else j][j if i > j else i])
                res += 1
